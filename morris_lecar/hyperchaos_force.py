...
from supervisors import HyperChaoticAttractor
from ml_block import MorrisLecarBlockGPU, minmax_transform 
...
def main():
    ...
    t = np.arange(0, T, dt)
    nt = t.size
    x = HyperChaoticAttractor(T, dt, tau=.02).generate()
    ...
    try:
        model = MorrisLecarBlockGPU(supervisor=signal, BIAS=current, T=T, dt=dt, 
    ...
    plot_model(model, "hyperrossler", random_neurons, voltage_trace, 
               decoder_trace, **plot_params)
