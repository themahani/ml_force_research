...
class Izhikevich:
    def __init__(
        ...
        self._w = (
            ...
            * (np.random.rand(N, N) < p)
            / (p * np.sqrt(N))
        )  # weight matrix. Constant for now.
    ...
    def _v_dot(self):
        ...
        return (
            self.k * (self.v - self.v_r) * (self.v - self.v_t) - self.u + self.ipsc
        ) / self.C
    ...
    def render(self, rls_start, rls_stop, rls_step, n_neurons):
        ...
        if n_neurons > self._N:
            raise ValueError(
                f"n_neurons = {n_neurons} should be less than or equal to {self._N}"
            )
...
def main():
    ...
    voltage_trace = model.render(
        rls_start=5000, rls_stop=10000, rls_step=20, n_neurons=10
    )
    # plot the decoded signal vs the original input
    plt.plot(t, signal[:, 0], lw=0.5)
    plt.plot(t, model.x_hat_rec[:, 0], lw=0.5)
